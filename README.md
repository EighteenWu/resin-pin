...
Resin 旁边的 sidecar：每天把节点池里 **TW / JP / HK / SG** 的可用节点钉成单节点 Platform，并提供可复制的 HTTP 正向代理链接。
...
1. 筛选启用、有出站、未熔断、已有出口 IP 的 `tw/jp/hk/sg` 节点。
...
打开 `http://服务器:2270`，用 **Resin Admin Token** 登录。
...
页面点「一键复制可用」，到管理员代理池逐条或批量粘贴。一键复制只包含当前状态为「可用」的链接。
